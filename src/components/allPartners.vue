<template>
  <div class="container">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      <div class="container">
        <h3 class="navbar-brand">Channel Partners</h3>
        <button
        ...
        >
        <ul class="navbar-nav">
          <li class="nav-link" @click="addNewPartner()">Add Channel Partner</li>
        </ul>
          <ul class="navbar-nav">
      ...
      <table class="table">
        <thead>
          <tr>
            ...
            <th class="table-head">All</th>
            <th class="table-head">Edit</th>
          </tr>
        </thead>
        <tbody>
          <template v-for="user in tableData">
            <tr v-if="user.role === 'supervisor'" :key="user.email">
            <td>{{user.fullName}}</td>
            <td>{{user.email}}</td>
            <td>{{user.status}}</td>
            <td>{{user.region}}</td>
            <td>{{user.created}}</td>
            <td @click="specificPartner(user)">
